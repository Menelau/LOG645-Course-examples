/* Computing Pi using the formula:
 *
 *              pi = 4*[1 - 1/3 + 1/5 - 1/7 + 1/9 - . . . ]
 *
 * This version solves the racing condition using the Busy Waiting method.
 *
...
void* Thread_Sum(void* rank) {
    ...
    for (i = my_first_i; i < my_last_i; i++, factor = -factor){
        while (flag != my_rank);    // careful with compiler's optimization
        sum += factor / (2 * i + 1);
