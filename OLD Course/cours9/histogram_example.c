/*
 ============================================================================
 Name        : LOG645_H18_C07_Hist.c
 Author      : Peter Pacheco (Introduction to parallel programming)
 Version     :
...
void Barrier(void);
void Get_my_start_finish(long, int, int, int*, int*);
...
void *Thread_work(void* rank) {
   ...
   for (i = start; i < finish; i++) {
      ...
      loc_bin_counts[offset+bin]++;
   }

#ifdef DEBUG
   printf("My rank = %ld, start = %d, finish = %d\n", my_rank, start, finish);
   for(i=0;i<bin_count;i++)
      printf("Lbc[%ld,%d] = %d ",my_rank,i,loc_bin_counts[i]);
   printf("\n");
   fflush(stdout);
#endif
...
#ifdef DEBUG
   printf("My rank = %ld, got to the barrier\n", my_rank);
   fflush(stdout);
#endif
...
void Get_my_start_finish(long my_rank, int count, int thread_count,
   ...
   if (my_rank < fraction) {
      (*start_p) = (int)((loc_count+1) * my_rank);
      *finish_p = *start_p + 1 + loc_count;
   } else {
      (*start_p) = ((loc_count+1)*fraction)+((my_rank-fraction)*loc_count);
      *finish_p = *start_p + loc_count;
   }
}  /* Get_my_start_finish */
